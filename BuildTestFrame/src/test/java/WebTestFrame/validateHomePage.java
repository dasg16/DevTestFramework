...
public class validateHomePage extends base {
	...
	public void checkLinksToFacebook() throws IOException {
		...
		for (int i = 0; i < count; i++) {
			...
			int j = i;
			int numOfTimesFB = 0;
			if (test.toLowerCase().contains("facebook")) {

				numOfTimesFB = numOfTimesFB + 1;
				...
				// Validate facebook page link on HomePage
				Assert.assertEquals(driver.getCurrentUrl(), "https://www.facebook.com/QAClickAcademy");
				log.info("Validated QAcademy's facebook page link on HomePage");
